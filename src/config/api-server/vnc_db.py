...
class VncDbClient(object):
    ...
    def update_vn_id_pool(self):
        (ok, result, mask) = self._object_db.object_list("global_vrouter_config")
        if not ok:
            return
        gvc_uuid = [result[0][1]]
        (ok, vxlan_range_info) = self._object_db.object_read("global_vrouter_config", gvc_uuid)
        if not ok:
            return

        vxlan_id_range = vxlan_range_info[0].get("vxlan_id_range", None)
        vxlan_start_id = vxlan_id_range["start_id"] if vxlan_id_range else None
        vxlan_end_id = vxlan_id_range["end_id"] if vxlan_id_range else None

        if not vxlan_start_id:
            vxlan_start_id = 1
        if not vxlan_end_id:
            vxlan_end_id = self._zk_db._VN_MAX_ID+1
