...
class VncApiServer(object):
    ...
    def __init__(self, args_str=None):
        ...
        # reset vxlan range, now just support one passage for a SDN system.
        self._vxlan_range_reset(self._db_conn)
   ...
   # end
    ...
    # vxlan range reset
    def _vxlan_range_reset(self, db_connect):
        (ok, result, mask) = db_connect._object_db.object_list("global_vrouter_config")
        if not ok:
            return
        gvc_uuid = [result[0][1]]
        (ok, vxlan_range_info) = db_connect._object_db.object_read("global_vrouter_config", gvc_uuid)
        if not ok:
            return

        vxlan_id_range = vxlan_range_info[0].get("vxlan_id_range", None)
        if vxlan_id_range is None:
            return

        vxlan_start_id = vxlan_id_range.get("start_id", None)
        vxlan_end_id = vxlan_id_range.get("end_id", None)

        if not vxlan_start_id:
            vxlan_start_id = 1
        if not vxlan_end_id:
            vxlan_end_id = cassandra._VN_MAX_ID+1

        max_alloc = vxlan_end_id - vxlan_start_id
        if (max_alloc <= 0):
            return

        zk_db = self._db_conn._zk_db
        if (zk_db._vn_id_allocator._start_idx+1 == vxlan_start_id and
            zk_db._vn_id_allocator._max_alloc == max_alloc+1):
            return

        zk_db._vn_id_allocator = IndexAllocator(
            zk_db._zk_client, zk_db._zk_path_pfx+zk_db._VN_ID_ALLOC_PATH, max_alloc, vxlan_start_id-1)
    # end range reset
...
def init_zookeeperclient(args_str):
    global zookeeper_client
    if not args_str:
        args_str = ' '.join(sys.argv[1:])
    _api_args,_ = utils.parse_args(args_str)
    zookeeper_client = ZookeeperClient("contrail-api", _api_args.zk_server_ip)

def main(args_str=None, server=None):
    init_zookeeperclient(args_str)
