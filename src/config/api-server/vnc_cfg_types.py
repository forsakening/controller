...
class FloatingIpServer(Resource, FloatingIp):
    ...
    @classmethod
    def _get_fip_pool_version(cls, vn_fq_name, db_conn):
        version = None

        vn_uuid = db_conn.fq_name_to_uuid('virtual_network', vn_fq_name)
        ok, res = cls.dbe_read(db_conn, 'virtual_network', vn_uuid)
        if ok:
            # Successful read returns virtual network.
            vn_dict = res
            ...
            for ipam in ipam_refs:
                ...
                for ipam_subnet in ipam_subnets:
                    ...
                    if subnet_dict and subnet_dict.get('ip_prefix'):
                        ip_version = netaddr.IPAddress(subnet_dict.get('ip_prefix')).version
                        return True, version
    ...
    def pre_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ...
        try:
            ...
            if not fip_subnets:
                ...
                ip_version = None
                if req_ip:
                    ip_version = netaddr.IPAddress(req_ip).version
                else:
                    ok, ret_val = cls._get_fip_pool_version(vn_fq_name, db_conn)
                    if ok:
                        ip_version = ret_val
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn,
        ...
        if 'virtual_machine_interface_properties' in obj_dict:
            ...
            old_vlan = int((read_result.get('virtual_machine_interface_properties') or {})
                            .get('sub_interface_vlan_tag') or 0)
            if new_vlan != old_vlan and new_vlan != 0 and old_vlan != 0:
