...
class GlobalVrouterConfigProvisioner(object):
    ...
    def __init__(self, args_str=None):
        ...
        if self._args.oper != "add":
            ...
            conf_obj=GlobalVrouterConfig()
            if self._args.flow_export_rate:
                conf_obj.set_flow_export_rate(self._args.flow_export_rate)
            if self._args.vnid_start and self._args.vnid_end:
                vxlan_id_range = VxlanIdRange(start_id=self._args.vnid_start, end_id=self._args.vnid_end)
                conf_obj.set_vxlan_id_range(vxlan_id_range)
    ...
    def _parse_args(self, args_str):
        ...
        parser.add_argument(
            "--flow_export_rate", type=int,
            help="Flow export rate to the collector")
        parser.add_argument(
            "--vnid_start", type=int,
            help="vxlan range start id")
        parser.add_argument(
            "--vnid_end", type=int,
            help="vxlan range end id")
        parser.add_argument(
