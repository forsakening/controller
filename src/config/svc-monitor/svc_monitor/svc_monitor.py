...
class SvcMonitor(object):
    ...
    def post_init(self, vnc_lib, args=None):
        ...
        vm_cls = {
            'openstack':'svc_monitor.virtual_machine_manager.VirtualMachineManager',
            'vcenter':'svc_monitor.virtual_machine_manager.VmwareVirtualMachineManager',
        }
        self.vm_manager = importutils.import_object(
            vm_cls[self._args.orchestrator],
    ...
    def _delete_shared_vn(self, vn_uuid):
        ...
        if self._args.orchestrator=="openstack":
            try:
                ...
                pass
        elif self._args.orchestrator=="vcenter":
            try:
                self.vm_manager.delete_unused_vn(vn_uuid)
            except Exception as e:
                self.logger.info("Deleting vn %s failed, exception %s" % (vn_uuid,e))
    ...
    for project in ProjectSM.values():
        ...
        for vn_id in vn_id_list:
            ...
            if vn.name in svc_info.get_shared_vn_list():
                ...
                for vn_prefix in svc_info.get_shared_vn_list():
                    if vn.name.startswith(vn_prefix):
                        monitor._delete_shared_vn(vn.uuid)
                        break
...
def parse_args(args_str):
    ...
    if args.conf_file:
        ...
        if 'VCENTER' in config.sections():
            schedops.update(dict(config.items("VCENTER")))
    ...
    parser.add_argument("--vcenter_protocol",
                        help="Protocol of vcenter server")
    parser.add_argument("--vcenter_host",
                        help="IP address of vcenter server")
    parser.add_argument("--vcenter_user",
                        help="Username known to vcenter server")
    parser.add_argument("--vcenter_pwd",
                        help="Password known to vcenter server")
    parser.add_argument("--vcenter_cluster",
                        help="Cluster known to vcenter server")
    parser.add_argument("--vcenter_datacenter",
                        help="Datacenter known to vcenter server")
    parser.add_argument("--vcenter_dvswitch",
                        help="DvSwitch known to vcenter server")
    parser.add_argument("--vcenter_port",
                        help="Port known to vcenter server")
    parser.add_argument("--orchestrator",
                        help="orchestrator of the system")
