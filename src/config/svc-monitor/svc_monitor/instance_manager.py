...
class InstanceManager(object):
    ...
    def _get_default_security_group(self, vn):
        sg_fq_name = self._check_create_service_sg(vn)
        return self._vnc_lib.security_group_read(fq_name=sg_fq_name)
    ...
    def _upgrade_config(self, st, si):
        ...
        other0_vn = si.params.get('other0_virtual_network', None)
        ...
        for index in range(0, len(st_if_list)):
            ...
            elif st_if_type == svc_info.get_other0_if_str():
                itf = ServiceInstanceInterfaceType(virtual_network=other0_vn)
            itf_list.append(itf)
    ...
    def validate_network_config(self, st, si):
        ...
        for index in range(0, len(st_if_list)):
            ...
            if not vn_fq_str:
                ...
                vn_fq_name = self._vnc_lib.id_to_fq_name(vn_id)
            else:
                try:
                    ...
                    self.logger.error(
                        "network %s not found for service-instance %s"%(vn_fq_str,si.uuid))
    ...
    def _check_create_service_sg(self, obj):

        # search or create default svc sg
        svc_sg_name = svc_info.get_default_sg()

        sg_fq_name = obj.fq_name[:-1] + [svc_sg_name]
        try:
            sg_id = self._vnc_lib.fq_name_to_id(
                'security-group', sg_fq_name)
        except NoIdError:
            sg_id = self.create_service_sg(svc_sg_name,obj.fq_name[:-1])

        return sg_fq_name

    def create_service_sg(self,sg_name, proj_fq_name, user_visible=None):
        self.logger.info(
            "Creating security group %s %s" % (":".join(proj_fq_name),sg_name))
        ...
        if user_visible is not None:
            id_perms = IdPermsType(enable=True,
                            ...
                            description="For nfv use, do not update or delete")
        else:
            id_perms = IdPermsType(enable=True,
                            description="For nfv use, do not update or delete")
        sg_obj.set_id_perms(id_perms)

        policy_entry = PolicyEntriesType()
        subnet_default = SubnetType(ip_prefix = '0.0.0.0', ip_prefix_len = 0)
        addr1 = AddressType(security_group="local",
                               subnet = None,
                               network_policy = None,
                               virtual_network= None)
        addr2 = AddressType(security_group=None,
                        subnet= subnet_default,
                        network_policy= None,
                        virtual_network= None)
        port = PortType(end_port= 65535, start_port= 0)
        rule1 = PolicyRuleType(direction= u'>',
                               protocol= u'any',
                               dst_addresses= [addr2],
                               action_list= None,
                               dst_ports= [port],
                               ...
                               rule_sequence= None,
                               src_ports= [port])
        rule2 = PolicyRuleType(direction= u'>',
                               protocol= u'any',
                               dst_addresses= [addr1],
                               action_list= None,
                               dst_ports= [port],
        ...
        policy_entry.add_policy_rule(rule1)
        policy_entry.add_policy_rule(rule2)

        sg_obj.set_security_group_entries(policy_entry)
        try:
            self._vnc_lib.security_group_create(sg_obj)
        except RefsExistError:
            sg_obj = self._vnc_lib.security_group_read(
                fq_name=sg_obj.get_fq_name())
    ...
    def get_svc_vn_info(self, itf_type, si):
        ...
        service_vn_name += si.uuid[0:7]
        return service_vn_name, service_vn_subnet, service_vn_subnet6

    def _check_create_service_vn(self, itf_type, si):
        vn_id = None
        service_vn_name, service_vn_subnet, service_vn_subnet6 = self.get_svc_vn_info(itf_type, si)
        if itf_type not in ("manage"):
            service_vn_name += si.uuid[0:7]
        ...
        try:
            ...
            vn_id = self.create_service_vn(service_vn_name,
                service_vn_subnet, None, si.fq_name[:-1])
    ...
    def _check_create_netns_vm(self, instance_index, si, st, vm):
        ...
        # for logical router interface indicated
        gw_itf_obj = None
        if si.logical_router:
            logical_router = LogicalRouterSM.get(si.logical_router)
            if logical_router and logical_router.gateway_interface:
                if not VirtualMachineInterfaceSM.get(logical_router.gateway_interface):
                    pass
                gw_itf = VirtualMachineInterfaceSM.read_obj(logical_router.gateway_interface)
                gw_itf_obj = VirtualMachineInterface.from_dict(**gw_itf)
                # set security group
                sg_obj = self._get_default_security_group(gw_itf_obj)
                gw_itf_obj.set_security_group(sg_obj)

        for nic in si.vn_info:
            if nic["type"] == "right" and gw_itf_obj:
                vmi_obj = gw_itf_obj
                vmi_obj.set_virtual_machine(vm_obj)
                self._vnc_lib.virtual_machine_interface_update(gw_itf_obj)
            else:
