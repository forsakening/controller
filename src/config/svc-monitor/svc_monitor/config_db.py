    ...
    def update(self, obj=None):
        ...
        if self.service_instance:
            if len(display_list) == 5:
                ...
                # when nfv name end with _, the index will be like _1,
                # which causes error,
                # so replace _ with empty.
                self.index = int(display_list[-2].replace("_","")) - 1
            else:
                self.index = -1
...
class LogicalRouterSM(DBBaseSM):
    ...
    def update(self, obj=None):
        ...
        lre = obj.get("logical_router_entries",None)
        if lre:
            self.gateway_interface = lre.get("gateway_interface", None)
        else:
