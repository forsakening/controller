...
try:
    from neutron.common.constants import PROTO_NAME_ICMP,PROTO_NUM_ICMP,PROTO_NAME_TCP,PROTO_NAME_UDP,NET_STATUS_ACTIVE,NET_STATUS_DOWN,PORT_STATUS_ACTIVE,DEVICE_OWNER_ROUTER_INTF,DEVICE_OWNER_ROUTER_GW,PORT_STATUS_DOWN,PROTO_NUM_TCP,PROTO_NUM_UDP
except ImportError:
    from neutron_lib.constants import PROTO_NAME_ICMP,PROTO_NUM_ICMP,PROTO_NAME_TCP,PROTO_NAME_UDP,NET_STATUS_ACTIVE,NET_STATUS_DOWN,PORT_STATUS_ACTIVE,DEVICE_OWNER_ROUTER_INTF,DEVICE_OWNER_ROUTER_GW,PORT_STATUS_DOWN,PROTO_NUM_TCP,PROTO_NUM_UDP
...
class VnidAllocator(IndexAllocator):
    def alloc(self, value=None, idx=None):
        if not idx:
            return super(VnidAllocator, self).alloc(value)

        # TODO there must be contrail-exception
        if idx< len(self._in_use) and self._in_use[idx]!=0:
            raise ResourceExistsError()
        self._set_in_use(idx)

        # TODO there may be allowed if idx out of _alloc_list
...
class DBInterface(object):
    ...
    def _allocate_vnid(self, vn_name, vnid):
        _vn_id_allocator = VnidAllocator(
            vnc_cfg_api_server.zookeeper_client, SchemaTransformerDB._VN_ID_ALLOC_PATH, SchemaTransformerDB._VN_MAX_ID)
        return _vn_id_allocator.alloc(vn_name, vnid-1)
    ...
    def _network_neutron_to_vnc(self, network_q, oper):
        ...
        # allocate vxlan id if indicated
        if network_q.get('provider:network_type')=="vxlan" and seg_id:
            # allocate vnid auto
            project_id = str(uuid.UUID(network_q.get('project_id')))
            fq_name = self._vnc_lib.id_to_fq_name(project_id)
            fq_name.append(net_name)
            net_fq_name_str = ":".join(fq_name)
            self._allocate_vnid(net_fq_name_str, seg_id)
            net_obj.set_virtual_network_network_id(seg_id)
            # allocate vnid manually
            vn_prop = net_obj.get_virtual_network_properties()
            if not vn_prop:
                vn_prop = VirtualNetworkType()
            vn_prop.set_vxlan_network_identifier(seg_id)
            net_obj.set_virtual_network_properties(vn_prop)
        ...
        if 'qos_policy_id' in network_q:
            if network_q['qos_policy_id']:
                qp_obj = self._vnc_lib.qos_policy_read(id=network_q['qos_policy_id'])
                net_obj.set_qos_policy(qp_obj)
            elif not network_q['qos_policy_id'] and oper == UPDATE:
                qp_refs = net_obj.get_qos_policy_refs()
                if qp_refs:
                    for qp_ref in qp_refs:
                        qp = self._vnc_lib.qos_policy_read(id=qp_ref['uuid'])
                        net_obj.del_qos_policy(qp)
    ...
    def _network_vnc_to_neutron(self, net_obj, oper=READ, context=None):
        ...
        qos_policy_refs = net_obj.get_qos_policy_refs()
        if qos_policy_refs:
            net_q_dict['qos_policy_id'] = qos_policy_refs[0]['uuid']
        else:
            net_q_dict['qos_policy_id'] = None
    ...
    def _floatingip_neutron_to_vnc(self, context, fip_q, oper):
        ...
        if 'qos_policy_id' in fip_q:
            if fip_q['qos_policy_id']:
                qp_obj = self._vnc_lib.qos_policy_read(id=fip_q['qos_policy_id'])
                fip_obj.set_qos_policy(qp_obj)
            elif not fip_q['qos_policy_id'] and oper == UPDATE:
                qp_refs = fip_obj.get_qos_policy_refs()
                if qp_refs:
                    for qp_ref in qp_refs:
                        qp = self._vnc_lib.qos_policy_read(id=qp_ref['uuid'])
                        fip_obj.del_qos_policy(qp)
    ...
    def _floatingip_vnc_to_neutron(self, fip_obj, memo_req=None, oper=READ):
        ...
        qos_policy_refs = fip_obj.get_qos_policy_refs()
        if qos_policy_refs:
            fip_q_dict['qos_policy_id'] = qos_policy_refs[0]['uuid']
        else:
            fip_q_dict['qos_policy_id'] = None
    ...
    def _port_neutron_to_vnc(self, port_q, net_obj, oper):
        ...
        if 'qos_policy_id' in port_q:
            if port_q['qos_policy_id']:
                qp_obj = self._vnc_lib.qos_policy_read(id=port_q['qos_policy_id'])
                port_obj.set_qos_policy(qp_obj)
            elif not port_q['qos_policy_id'] and oper == UPDATE:
                qp_refs = port_obj.get_qos_policy_refs()
                if qp_refs:
                    for qp_ref in qp_refs:
                        qp = self._vnc_lib.qos_policy_read(id=qp_ref['uuid'])
                        port_obj.del_qos_policy(qp)
        return port_obj
	...
	address_pairs = []
        ...
        qos_policy_refs = port_obj.get_qos_policy_refs()
        if qos_policy_refs:
            port_q_dict['qos_policy_id'] = qos_policy_refs[0]['uuid']
        else:
            port_q_dict['qos_policy_id'] = None
