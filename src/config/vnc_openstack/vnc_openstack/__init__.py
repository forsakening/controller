...
class OpenstackDriver(vnc_plugin_base.Resync):
    def __init__(self, api_server_ip, api_server_port, conf_sections, sandesh):
        ...
        if api_server_ip == '0.0.0.0':
            ...
            #guwei
            try:
                if IPy.IP(api_server_ip).version() == 6:
    ...
    def _resync_domains_projects_forever(self):
        ...
        def _is_not_reserved_project(project_fq_name, default_proj_fq_name):
            if self._contrail_vcenter_mode == "vcenter_only":
                return (project_fq_name != default_proj_fq_name
                       and project_fq_name != ['default-domain','vCenter'])
            else:
                return project_fq_name != default_proj_fq_name
        ...
        try:
            ...
            vnc_project_ids = set([proj['uuid'] for proj in vnc_all_projects
                if _is_not_reserved_project(proj['fq_name'], default_proj_fq_name)])
...
class NeutronApiDriver(vnc_plugin_base.NeutronApi):
    def __init__(self, api_server_ip, api_server_port, conf_sections, sandesh, **kwargs):
        ...
        # Bottle callbacks for qos-policy operations
        self.route('/neutron/qos_policy',
                   'POST', self._npi.plugin_http_post_qospolicy)

        # Bottle callbacks for qos-bandwidth-limit-rule
        self.route('/neutron/qos_bandwidth_limit_rule',
                   'POST', self._npi.plugin_http_post_qosbandwidthlimitrule)

        # Bottle callbacks for qos-bandwidth-limit-rule
        self.route('/neutron/qos_dscp_marking_rule',
                   'POST', self._npi.plugin_http_post_qosdscpmarkingrule)
