...
class BgpRouterDM(DBBaseDM):
    ...
    def __init__(self, uuid, obj_dict=None):
        ...
        self.bgp_remote_neighbours = None
    ...
    def update(self, obj=None):
        ...
        self.bgp_remote_neighbours = obj.get('bgp_router_remote_neighbours') or {}
...
class PhysicalRouterDM(DBBaseDM):
    ...
    def update(self, obj=None):
        ...
        ''' store in attr or ? 
            attr: {
                 'options': [
                    {
                        'mode': 'l2gw',
                        'interface_name': '10gei-1/1/1',
                    },
                    {
                        'mode': 'l3gw',
                        'interface_name': '10gei-1/1/1',
                        'ipaddr': '172.16.161.208/25',
                        'vlan-tag': 294,
                    },
                    {
                        'mode': 'l2dci',
        ...
        '''
        self.attach_vn_lr = {}
        for vn_obj in obj.get('virtual_network_refs', []):
            if 'uuid' in vn_obj and 'attr' in vn_obj and vn_obj['attr']:
                self.attach_vn_lr[vn_obj['uuid']] = vn_obj['attr']
    ...
    def get_vn_lr_modes(self, uuid):
        options = self.attach_vn_lr[uuid]
    ...
    def get_vrouter_ip(self):
        vrouter_ip = []
        try:
            vrouter_list = self._manager._vnc_lib.virtual_routers_list().get("virtual-routers")
        except:
            self._logger.error("could not get vrouter list.")

        for vrouter in vrouter_list:
            fq_name = vrouter.get('fq_name')
            try:
                vrouter_obj = self._manager._vnc_lib.virtual_router_read(fq_name = fq_name)
                vrouter_ip.append(str(vrouter_obj._virtual_router_ip_address))
            except:
                self._logger.error("could not get vrouter ip.")
...
class VirtualNetworkDM(DBBaseDM):
    ...
    def __init__(self, uuid, obj_dict=None):
        ...
        # add l2dci_connects attach_vlan irb
        self.l2_dci_connects = set()
        self.attach_vlan = None
        self.irb = None
        self.update(obj_dict)
    # end __init__
    ...
    def update(self, obj=None):
        ...
        self.irb = DMUtils.get_network_irb(obj.get('network_ipam_refs', []))

        # attach vlan
        self.attach_vlan = obj.get('attach_network', None)

        # need get physical router attr ?
        # physical_router_options
        self.physical_routers_options = {}
        for pr in obj.get('physical_router_back_refs', []):
            if pr.get('uuid', '') and pr.get('attr', {}):
                self.physical_routers_options[pr['uuid']] = pr['attr']

    # end update

    def get_options_from_pr(self, uuid):
        if uuid in self.physical_routers_options:
            return self.physical_routers_options['uuid']
        else:
            return None
    # end get options from physical router
