...
Collector::Collector(EventManager *evm, string server_addr, short server_port,
    ...
    IpAddress server_address = IpAddress::from_string(server_addr,ec);
    if (ec.value() != 0){
        LOG(ERROR, "server_addr is error.\n");
        return;
    }
    SandeshServer::Initialize(server_address, server_port);
