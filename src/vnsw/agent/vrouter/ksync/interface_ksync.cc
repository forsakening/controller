...
InterfaceKSyncEntry::InterfaceKSyncEntry(InterfaceKSyncObject *obj,
    ...
    qos_dscp_(entry->qos_dscp_),
    mtu_(entry->mtu_),
    multicast_vrf_id_(entry->multicast_vrf_id_),
    inbound_cir_(entry->inbound_cir_),
    inbound_pir_(entry->inbound_pir_),
    inbount_cbs_(entry->inbount_cbs_),
    inbount_ebs_(entry->inbount_ebs_),
    inbount_pbs_(entry->inbount_pbs_),
    outbound_cir_(entry->outbound_cir_),
    outbound_pir_(entry->outbound_pir_),
    outbount_cbs_(entry->outbount_cbs_),
    outbount_ebs_(entry->outbount_ebs_),
    outbount_pbs_(entry->outbount_pbs_),
...
InterfaceKSyncEntry::InterfaceKSyncEntry(InterfaceKSyncObject *obj,
    ...
    vrf_id_(intf->vrf_id()),
    qos_dscp_(VmInterface::KDefDscp),
    mtu_(VmInterface::kDefMtu),
...
std::string InterfaceKSyncEntry::ToString() const {
    std::stringstream s;
    s << "Interface : " << interface_name_ << " Index : " << interface_id_<<"Mtu : "<<mtu_;
    const VrfEntry* vrf =
...
bool InterfaceKSyncEntry::Sync(DBEntry *e) {
    ...
    if (intf->type() == Interface::VM_INTERFACE) {
        ...
        if (qos_dscp_ != vm_port->qos_dscp()) {
            qos_dscp_ = vm_port->qos_dscp();
            ret = true;
        }

        if (inbound_cir_ != vm_port->qos_inbound()) {
            inbound_cir_ = vm_port->qos_inbound();
            ret = true;
        }

        if (inbount_cbs_ != vm_port->qos_in_burst()) {
            inbount_cbs_ = vm_port->qos_in_burst();
            ret = true;
        }

        if (outbound_cir_ != vm_port->qos_outbound()) {
            outbound_cir_ = vm_port->qos_outbound();
            ret = true;
        }

        if (outbount_cbs_ != vm_port->qos_out_burst()) {
            outbount_cbs_ = vm_port->qos_out_burst();
            ret = true;
        }
...
int InterfaceKSyncEntry::Encode(sandesh_op::type op, char *buf, int buf_len) {
    ...
    encoder.set_vifr_replace_dscp((uint8_t)qos_dscp_);
    ...
    encoder.set_vifr_mtu(mtu_);
    encoder.set_vifr_inbound_meter_mode(vr_meter_mode::e_VR_METER_SRTCM_COLOR_BLIND);
    encoder.set_vifr_inbound_cir_kbs(inbound_cir_);
    encoder.set_vifr_inbound_cbs(inbount_cbs_ == 0 ? inbound_cir_ * 0.8 : inbount_cbs_);
    encoder.set_vifr_outbound_meter_mode(vr_meter_mode::e_VR_METER_SRTCM_COLOR_BLIND);
    encoder.set_vifr_outbound_cir_kbs(outbound_cir_);
    encoder.set_vifr_outbound_cbs(outbount_cbs_ == 0 ? outbound_cir_ * 0.8 : outbount_cbs_);
