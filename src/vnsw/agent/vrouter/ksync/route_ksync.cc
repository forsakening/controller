...
bool RouteKSyncEntry::Sync(DBEntry *e) {
    ...
    if (rt_type_ == Agent::INET4_UNICAST || rt_type_ == Agent::INET6_UNICAST) {
        ...
        } else if (NULL != vn && obj->RouteNeedsMacBindingL3Vxlan(uc_rt)) {
            IpAddress gateway = vn->GetDefaultRouteFromVnHostRoutes();
            if (gateway.to_string() == "0.0.0.0") {
                 gateway = vn->GetL3VxlanRouteGateway();
                 LOG(DEBUG, " routeL3VxlanRouteGateway "<<gateway.to_string()<<endl);
            }
            mac = obj->GetIpMacBinding(uc_rt->vrf(), gateway, uc_rt);
    ...
    if (rt_type_ != Agent::BRIDGE) {
        ...
        if (mac_ != MacAddress::ZeroMac()) {
            if ((addr_.is_v4() && prefix_len_ != 32) ||
                ...
                LOG(DEBUG, "Unexpected MAC stitching for route "
                    << ToString());
            }
...
/* parameter ip is L3Vxlan route next-hop*/
void VrfKSyncObject::NotifyL3VxlanRoute(VrfEntry *vrf, VrfState *state, 
                                           const IpAddress &ip) {
    uint32_t   plen = 0;
    IpAddress  l3_vxlan_gateway;
    IpAddress  l3_vxlan_static_gateway;
    ...
    l3_vxlan_gateway = vrf->vn()->GetDefaultRouteFromVnHostRoutes();
    l3_vxlan_static_gateway = vrf->vn()->GetL3VxlanRouteGateway();
    if ((l3_vxlan_gateway.to_string() == "0.0.0.0")
       && (l3_vxlan_static_gateway.to_string() == "0.0.0.0")) {
        return;
    }
    if (ip == l3_vxlan_gateway) {
        LOG(DEBUG,"NotifyL3VxalnRouteGateway "<<l3_vxlan_gateway.to_string()<<" ip "<<ip.to_string()<<endl);
    } else if (ip == l3_vxlan_static_gateway) {
        LOG(DEBUG,"NotifyL3VxlanStaticRouteGateway "<<l3_vxlan_gateway.to_string()<<" ip "<<ip.to_string()<<endl);
        if (!vrf->vn()->GetL3VxlanRouteDestByGateway(&prefix_addr, &plen, ip)) {
            return;
        }
    }
    table = vrf->GetInet4UnicastRouteTable();
    ...
    LOG(DEBUG,"syncRoute "<<rt->ToString()<<endl);
    const NextHop *nh = rt->GetActiveNextHop();
    if (nh != NULL) {
        LOG(DEBUG,"type "<<nh->GetType()<<" policy "<<nh->PolicyEnabled());
        if (nh->GetType() == NextHop::INTERFACE) {
            LOG(DEBUG,"name "<<(static_cast<const InterfaceNH*>(nh))->GetInterface()->name()<<endl);
        } else {
            LOG(DEBUG,endl);
        }
    }
...
void VrfKSyncObject::AddIpMacBinding(VrfEntry *vrf, const IpAddress &ip,
    ...
    NotifyL3VxlanRoute(vrf, state, ip);
