...
bool ControllerVmRoute::AddChangePathExtended(Agent *agent, AgentPath *path,
    ...
    if (tunnel_bmap_ & (1 << TunnelType::DefaultType())) {
        new_tunnel_type = TunnelType::DefaultType();
    } else if ((tunnel_bmap_ == (1 << TunnelType::MPLS_GRE))
                && (TunnelType::DefaultType() == TunnelType::VXLAN)) {
        new_tunnel_type = TunnelType::DefaultType();
    } else {
        new_tunnel_type = TunnelType::ComputeType(tunnel_bmap_);
    }
    LOG(DEBUG,"controllervmroute: tunnel_bmap/label/plen/new_tunnel_type "<<tunnel_bmap_<<" label "<<label_<<" plen "<<(uint32_t)rt->plen()<<" "<<TunnelType::GetString(new_tunnel_type)<<endl);
#if 0
    ...
    if (tunnel_bmap_ == TunnelType::VxlanType()) {
        //Only VXLAN encap is sent, so label is vxlan or gre in vxlan l3 routing
