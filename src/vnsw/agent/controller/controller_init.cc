...
void VNController::XmppServerConnect() {
    ...
    while (count < MAX_XMPP_SERVERS) {
        ...
        if (!agent_->controller_ifmap_xmpp_server(count).empty()) {
            ...
            assert(ec.value() == 0);
            //TODO:guwei. 10 is inet6
            if (xmpp_cfg->endpoint.data()->sa_family == 10){
                xmpp_cfg->local_endpoint.address(
                        ip::address::from_string("::", ec));
                assert(ec.value() == 0);
            }
            /*
            xmpp_cfg->local_endpoint.address(
...
void VNController::DnsXmppServerConnect() {
    ...
    while (count < MAX_XMPP_SERVERS) {
        if (!agent_->dns_server(count).empty()) {
            ...
            assert(ec.value() == 0);
            //TODO:guwei. 10 is inet6
            if (xmpp_cfg_dns->endpoint.data()->sa_family == 10){
...
bool VNController::ApplyDnsReConfigInternal(std::vector<string> resp) {
    ...
    for (iter = resp.begin(); ++count < min_iter; iter++) {
        std::vector<string> srv;
        //guwei
        std::string dns_server_t = *iter;
        int last_idx = dns_server_t.find_last_of(":");
        dns_server_t.replace(last_idx,1,"-");
        boost::split(srv, dns_server_t, boost::is_any_of("-"));       
        //boost::split(srv, *iter, boost::is_any_of(":"));
