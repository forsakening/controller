...
int CfgFilter::GetIdPermsPropertyId(DBTable *table) const {
    ...
    if (table == agent_cfg_->cfg_qos_bw_table())
        return QosPolicy::ID_PERMS;
...
void CfgFilter::Init() {
    ...
    agent_cfg_->cfg_qos_bw_table()->RegisterPreFilter
       (boost::bind(&CfgFilter::CheckProperty, this, _1, _2, _3));
...
void CfgFilter::Shutdown() {
    ...
    agent_cfg_->cfg_qos_bw_table()->RegisterPreFilter(NULL);
