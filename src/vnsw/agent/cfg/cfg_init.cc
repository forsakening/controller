...
void AgentConfig::RegisterDBClients(DB *db) {
    ...
    cfg_qos_bw_table_ = (static_cast<IFMapAgentTable *>
        (IFMapTable::FindTable(agent_->db(), "qos-policy")));
    assert(cfg_qos_bw_table_);
    ...
    cfg_policy_management_table_ = (static_cast<IFMapAgentTable *>(
                IFMapTable::FindTable(agent_->db(), "policy-management")));
    assert(cfg_policy_management_table_);
    ...
    cfg_port_tuple_table_ = (static_cast<IFMapAgentTable *>
                            (IFMapTable::FindTable(agent_->db(),
                                                   "port-tuple")));
