...
VmInterface::VmInterface(const boost::uuids::uuid &uuid, 
    ...
    qos_dscp_(KDefDscp), mtu_(kDefMtu), dhcp_addr_(0), metadata_ip_map_(), hc_instance_set_(),
...
VmInterface::VmInterface(const boost::uuids::uuid &uuid,
    ...
    Interface(Interface::VM_INTERFACE, uuid, name, NULL), vm_(NULL, this),
    vn_(NULL), primary_ip_addr_(addr), subnet_bcast_addr_(0),
...
void VmInterface::ApplyConfig(bool old_ipv4_active, bool old_l2_active,
    ...
    // For SRIOV we dont generate any things like l2 routes, l3 routes etc
...
//QosPolicy routines
/////////////////////////////////////////////////
VmInterface::QosPolicyRuleEntry::QosPolicyRuleEntry() :
    ListEntry(), uuid_(nil_uuid()) {
}

VmInterface::QosPolicyRuleEntry::QosPolicyRuleEntry
    (const QosPolicyRuleEntry &rhs) :
        ListEntry(rhs.del_pending_), uuid_(rhs.uuid_) {
}

VmInterface::QosPolicyRuleEntry::QosPolicyRuleEntry(const uuid &u) :
    ListEntry(), uuid_(u) {
}
...
bool VmInterface::QosPolicyRuleEntry::operator ==
    (const QosPolicyRuleEntry &rhs) const {
    return uuid_ == rhs.uuid_;
}

bool VmInterface::QosPolicyRuleEntry::operator()
    (const QosPolicyRuleEntry &lhs, const QosPolicyRuleEntry &rhs) const {
    return lhs.IsLess(&rhs);
}

bool VmInterface::QosPolicyRuleEntry::IsLess
    (const QosPolicyRuleEntry *rhs) const {
    return uuid_ < rhs->uuid_;
}

void VmInterface::QosPolicyRuleEntry::Activate(VmInterface *interface) const {
    if (qos_bw_.get() != NULL)
        return;

    Agent *agent = static_cast<InterfaceTable *>
        (interface->get_table())->agent();
    QosBwKey QosBwKey(uuid_);
    qos_bw_ = static_cast<QosBwEntry *>
        (agent->qos_bw_table()->FindActiveEntry(&QosBwKey));
}

void VmInterface::QosPolicyRuleEntry::DeActivate(VmInterface *interface) const {
}

void VmInterface::QosPolicyRuleEntryList::Insert
    (const QosPolicyRuleEntry *rhs) {
    list_.insert(*rhs);
}

void VmInterface::QosPolicyRuleEntryList::Update
        (const QosPolicyRuleEntry *lhs, const QosPolicyRuleEntry *rhs) {
}

void VmInterface::QosPolicyRuleEntryList::Remove
        (QosPolicyRuleEntrySet::iterator &it) {
    it->set_del_pending(true);
}
...
bool VmInterface::FloatingIp::AddL2(const Agent *agent,
    ...
    evpn_table->AddLocalVmRoute(vmi->peer_.get(), vrf_->GetName(),
                                vmi->vm_mac(), vmi, floating_ip_, vmi->l2_label(), 
                                vn_name, sg_id_list, tag_id_list, path_preference, 
                                vrf_->vxlan_id(), vmi->etree_leaf(), vmi->name());

    evpn_table->AddLocalVmRoute(vmi->peer_.get(), vrf_->GetName(),
                                vmi->vm_mac(), vmi, IpAddress::from_string("0.0.0.0"), vmi->l2_label(), 
                                vn_name, sg_id_list, tag_id_list, path_preference, 
                                vrf_->vxlan_id(), vmi->etree_leaf(), vmi->name());
    return true;
}
...
bool VmInterface::FloatingIp::DeleteL2(const Agent *agent,
    ...
    evpn_table->DelLocalVmRoute(vmi->peer_.get(), vrf_->GetName(),
                                ...
                                vrf_->vxlan_id());
    evpn_table->DelLocalVmRoute(vmi->peer_.get(), vrf_->GetName(),
                                vmi->vm_mac(), vmi, IpAddress::from_string("0.0.0.0"),
                                vrf_->vxlan_id());
    return true;
}
