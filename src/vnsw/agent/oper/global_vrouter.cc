...
GlobalVrouter::GlobalVrouter(Agent *agent) :
    ...
    forwarding_mode_(Agent::L2_L3), flow_export_rate_(kDefaultFlowExportRate), qos_port_cnt(0),
...
void GlobalVrouter::GlobalVrouterConfig(IFMapNode *node) {
    ...
    if (node->IsDeleted() == false) {
        ...
        UpdateGlobalStaticRouteConfig(cfg->global_static_routes());
...
void GlobalVrouter::UpdateGlobalStaticRouteConfig(const GlobalStaticRouteList &route_list) {
    std::vector<GlobalStaticRoute> global_static_route_list;
    for (std::vector<autogen::RouteType>::const_iterator it = route_list.begin();
        it != route_list.end(); it ++) {
        int plen;
        IpAddress ip;
        boost::system::error_code ec;
        ...
        global_static_route_list.push_back(GlobalVrouter::GlobalStaticRoute(ip, plen, gw));
    }
    global_static_route_list_.swap(global_static_route_list);
    GlobalStaticRouteChangeNotify(&global_static_route_list, &global_static_route_list_);
}

void GlobalVrouter::GlobalStaticRouteChangeNotify(std::vector<GlobalStaticRoute> *old_value,
                                        std::vector<GlobalStaticRoute> *new_value){
    std::vector<GlobalStaticRoute>::iterator it_old = old_value->begin();
    std::vector<GlobalStaticRoute>::iterator it_new = new_value->begin();
    while (it_old != old_value->end()) {
        if (std::find(new_value->begin(), new_value->end(), *it_old) == new_value->end()) {
            DeleteGlobalStaticRoute(it_old);
        }
        it_old ++;
    }

    while (it_new != new_value->end()) {
       if (std::find(old_value->begin(), old_value->end(), *it_new) == old_value->end()) {
            AddGlobalStaticRoute(it_new);
       }
        it_new ++;
    }
}

void GlobalVrouter::AddGlobalStaticRoute(std::vector<GlobalStaticRoute>::iterator &it) {
    VnListType vn_list;
    vn_list.insert(agent()->fabric_vn_name());
...
void GlobalVrouter::DeleteGlobalStaticRoute(std::vector<GlobalStaticRoute>::iterator &it) {
    InetUnicastAgentRouteTable *table = agent()->fabric_inet4_unicast_table();
    table->DeleteReq(agent()->local_peer(), agent()->fabric_vrf_name(),
                           it->prefix_, it->plen_, NULL);
}
