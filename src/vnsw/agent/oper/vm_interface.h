...
public:
    ...
    static const uint32_t kDefMtu = 1500;
    static const int32_t KDefDscp = 0;
    ...
    struct FloatingIp : public ListEntry, VmInterfaceState {
       ...
       FloatingIp(const IpAddress &addr, const std::string &vrf,
                   const boost::uuids::uuid &vn_uuid,const IpAddress &ip, const int32_t inbound,
                   const int32_t outbound);
        virtual ~FloatingIp();
        ...
        int32_t qos_inbound;
        int32_t qos_outbound;
        int32_t qos_in_burst;
        int32_t qos_out_burst;
        int32_t qos_dscp_;
    ...
    struct QosPolicyRuleEntry : ListEntry {
        QosPolicyRuleEntry();
        QosPolicyRuleEntry(const QosPolicyRuleEntry &rhs);
        QosPolicyRuleEntry(const boost::uuids::uuid &uuid);
        virtual ~QosPolicyRuleEntry();

        bool operator == (const QosPolicyRuleEntry &rhs) const;
        bool operator() (const QosPolicyRuleEntry &lhs,
                         const QosPolicyRuleEntry &rhs) const;
        bool IsLess(const QosPolicyRuleEntry *rhs) const;
        void Activate(VmInterface *interface) const;
        void DeActivate(VmInterface *interface) const;

        mutable QosBwEntryRef qos_bw_;
        boost::uuids::uuid uuid_;
    };

    typedef std::set<QosPolicyRuleEntry, QosPolicyRuleEntry>
        QosPolicyRuleEntrySet;
    typedef std::vector<boost::uuids::uuid> QosBwUuidList;

    struct QosPolicyRuleEntryList {
        QosPolicyRuleEntryList() : list_() { }
        ~QosPolicyRuleEntryList() { }

        void Insert(const QosPolicyRuleEntry *rhs);
        void Update(const QosPolicyRuleEntry *lhs,
                    const QosPolicyRuleEntry *rhs);
        void Remove(QosPolicyRuleEntrySet::iterator &it);

        QosPolicyRuleEntrySet list_;
    };
    ...
    uint32_t qos_dscp() const {return qos_dscp_;}
    int qos_inbound() const {return qos_inbound_;}
    int qos_in_burst() const {return qos_in_burst_;}
    int qos_outbound() const {return qos_outbound_;}
    int qos_out_burst() const {return qos_out_burst_;}
    ...
    void UpdateVmPortQos(int qos_inbound, int qos_outbound, int qos_in_burst, int qos_out_burst,
                         int fip_inbound,int fip_outbound, int fip_in_burst, int fip_out_busrt);
    void UpdateVmPortQosVmware(int qos_inbound, int qos_outbound, const std::string &promisc_post);
    void InitVmPortQosVmware(const std::string &promisc_post);
    void DestroyVmPortQosVmware(const std::string &promisc_post);
...
private:
	...
	QosPolicyRuleEntryList qos_policy_rule_list_;
...
struct VmInterfaceConfigData : public VmInterfaceData {
    ...
    VmInterface::QosPolicyRuleEntryList qos_policy_rule_list_;
    ...
    uint16_t tx_vlan_id_;
    boost::uuids::uuid logical_interface_;
    int32_t qos_inbound_;
    int32_t qos_outbound_;
    int32_t qos_in_burst_;
    int32_t qos_out_burst_;
    int32_t qos_dscp_;
    uint32_t mtu_;
    VmiEcmpLoadBalance ecmp_load_balance_;
    IpAddress service_health_check_ip_;
