...
void ConfigManager::Init() {
    ...
    qos_bw_list_.reset(new ConfigManagerNodeList(agent_->qos_bw_table()));
...
int ConfigManager::Run() {
    ...
    count += qos_bw_list_->Process(max_count - count);
...
void ConfigManager::AddQosBwNode(IFMapNode *node) {
    qos_bw_list_->Add(agent_, this, node);
}
