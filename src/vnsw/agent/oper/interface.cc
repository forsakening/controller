...
void Interface::SetPciIndex(Agent *agent) {
    //flexvisor: if vmware interface, set pci index from the vmware related configuration
    std::istringstream pci;
    const PhysicalInterface *phy_intf =
            static_cast<const PhysicalInterface *>(this);
    if (PhysicalInterface::VMWARE == phy_intf->subtype())
    {
        LOG(INFO, "Vmware port, get pci:" << agent->params()->vmware_physical_interface_pci_addr());
        pci.str(agent->params()->vmware_physical_interface_pci_addr());
    }
    else
    {
        LOG(INFO, "Not vmware port, get pci:" << agent->params()->physical_interface_pci_addr());
        pci.str(agent->params()->physical_interface_pci_addr());
    }
...
void Interface::GetOsParams(Agent *agent) {
    ...
    if (transport_ == TRANSPORT_PMD) {
        if (type_ == PHYSICAL || 
            ...
            const PhysicalInterface *phy_intf = static_cast<const PhysicalInterface *>(this);
            if (PhysicalInterface::VMWARE == phy_intf->subtype()) {
                addr = ether_aton(agent->params()->vmware_physical_interface_mac_addr().c_str());
            }
            else {
                addr = ether_aton(agent->params()->physical_interface_mac_addr().c_str());
            }
...
void Interface::SetItfSandeshData(ItfSandeshData &data) const {
    ...
    case Interface::VM_INTERFACE: {
        ...
        data.set_mdata_ip6_addr(vintf->mdata_ip6_addr().to_string());
