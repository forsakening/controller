...
static void BuildQosPolicyList(VmInterfaceConfigData *data, IFMapNode *node) {
    QosPolicy *qos_policy_cfg = static_cast<QosPolicy *>
        (node->GetObject());
    ...
    CfgUuidSet(id_perms.uuid.uuid_mslong, id_perms.uuid.uuid_lslong,
               qos_policy_uuid);
    data->qos_policy_rule_list_.list_.insert
        (VmInterface::QosPolicyRuleEntry(qos_policy_uuid));
    LOG(DEBUG, "BuildQosPolicyList out "<<data->qos_outbound_<<" out_burst "<<data->qos_out_burst_<<" in "<<data->qos_inbound_<<" in_burst "<<data->qos_in_burst_);
    ...
    while (qos_policy_rule_it != qos_policy_cfg->qos_bandwidth_rule_entries().end()) {
        ...
        if (qos_policy_rule_it->direction == OUTBOUND_NAME) {
            if (qos_policy_rule_it->max_kbps >= 0) {
                data->qos_outbound_ = qos_policy_rule_it->max_kbps;
            }
            if (qos_policy_rule_it->max_burst_kbps >= 0) {
                data->qos_out_burst_ = qos_policy_rule_it->max_burst_kbps;
            }
        } else if (qos_policy_rule_it->direction == INBOUND_NAME) {
            if (qos_policy_rule_it->max_kbps >= 0) {
                data->qos_inbound_ = qos_policy_rule_it->max_kbps;
            }
            if (qos_policy_rule_it->max_burst_kbps >= 0) {
                data->qos_in_burst_ = qos_policy_rule_it->max_burst_kbps;
            }
        } else {
            assert(0);
        }
        qos_policy_rule_it++;
        LOG(DEBUG, "out "<<data->qos_outbound_<<" out_burst "<<data->qos_out_burst_<<" in "<<data->qos_inbound_<<" in_burst "<<data->qos_in_burst_);
    }

    QosDscpMarkingRuleEntryType dscp_marking_rule = qos_policy_cfg->qos_dscpmarking_rule_entry();
    LOG(DEBUG, "BuildQosPolicyList dscp "<<data->qos_dscp_);
    if (dscp_marking_rule.dscp_mark >= 0) {
        data->qos_dscp_ = dscp_marking_rule.dscp_mark;
        LOG(DEBUG, "dscp "<<data->qos_dscp_);
    }
}
...
bool InterfaceTable::VmiProcessConfig(IFMapNode *node, DBRequest &req,
    ...
    if (cfg->IsPropertySet(VirtualMachineInterface::MTU)) {
        data->mtu_ = (uint32_t)cfg->mtu();
        LOG(DEBUG,"vmiProcessConfig "<<node->name()<<" cfg mtu "<<data->mtu_);
    }
    ...
    for (DBGraphVertex::adjacency_iterator iter =
        ...
        if (adj_node->table() == agent_->cfg()->cfg_qos_bw_table()) {
            BuildQosPolicyList(data, adj_node);
        }
