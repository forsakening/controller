...
struct VnStaticRoute {
    IpAddress ip_prefix;
    uint32_t   plen;
    IpAddress  gw;
    std::vector<std::string> route_target;
    CommunityList community;
    VnStaticRoute(const IpAddress& ip, uint32_t len, const IpAddress& nh);
    bool IsV4() const {
        return ip_prefix.is_v4();
    }
    bool IsV6() const {
    ...
    bool operator<(const VnStaticRoute &rhs) const {
        if (ip_prefix != rhs.ip_prefix)
            return ip_prefix < rhs.ip_prefix;
...
struct VnData : public AgentOperDBData {
    ...
    VnData(const Agent *agent, IFMapNode *node, const string &name,
           ...
           const std::vector<VnStaticRoute> &static_route,
           int vxlan_id, int vnid, bool external, bool admin_state, bool enable_rpf,
           bool flood_unknown_unicast, int qos_inbound, int qos_outbound,
           int qos_in_burst, int qos_out_burst, int qos_dscp, int mtu, const uuid &qos_bw_id,
        ...
        static_route_(static_route),
        vxlan_id_(vxlan_id), vnid_(vnid), external_(external), admin_state_(admin_state),
        enable_rpf_(enable_rpf), flood_unknown_unicast_(flood_unknown_unicast),
        qos_inbound_(qos_inbound), qos_outbound_(qos_outbound),
        qos_in_burst_(qos_in_burst), qos_out_burst_(qos_out_burst),
        qos_dscp_(qos_dscp), mtu_(mtu), qos_bw_uuid_(qos_bw_id),
        ...
        VnData(const Agent *agent, IFMapNode *node, const string &name,
           ...
           const boost::uuids::uuid &qos_config_uuid, const uuid &qos_bw_id,
        ...
        vxlan_id_(vxlan_id), vnid_(vnid), external_(external), admin_state_(admin_state),
        enable_rpf_(enable_rpf), flood_unknown_unicast_(flood_unknown_unicast),
        qos_inbound_(-1), qos_outbound_(-1), qos_in_burst_(DEF_QOS_BW_BURST), 
        qos_out_burst_(DEF_QOS_BW_BURST),
        qos_dscp_(0), mtu_(DEF_MTU), qos_bw_uuid_(qos_bw_id),
        forwarding_mode_(forwarding_mode), qos_config_uuid_(qos_config_uuid),
        mirror_destination_(mirror_destination),
        pbb_etree_enable_(pbb_etree_enable), pbb_evpn_enable_(pbb_evpn_enable),
        layer2_control_word_(layer2_control_word), slo_list_(slo_list) {
    ...
    std::vector<VnStaticRoute> static_route_;
    int vxlan_id_;
    int vnid_;
    bool external_;
    bool admin_state_;
...
public:
    ...
    IpAddress GetDefaultRouteFromVnHostRoutes() const;    
    IpAddress GetL3VxlanRouteGateway() const;
    bool GetL3VxlanRouteDestByGateway(IpAddress *prefix_addr, uint32_t *plen, IpAddress gw) const;
    ...
    int GetQosInbound() const {return qos_inbound_;}
    int GetQosOutbound() const {return qos_outbound_;}
    int GetQosInBurst() const {return qos_in_burst_;}
    int GetQosOutBurst() const {return qos_out_burst_;}
    int GetQosDscp() const {return qos_dscp_;}
...
private:
    ...
    bool ApplySubnetRoute(Agent *agent, bool external);
