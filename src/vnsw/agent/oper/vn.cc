...
VnStaticRoute::VnStaticRoute(const IpAddress& ip, uint32_t len, const IpAddress& nh) {
    ip_prefix = ip;
    plen = len;
    gw = nh;
}

Ip4Address VnStaticRoute::GetBroadcastAddress() const {
    if (ip_prefix.is_v4()) {
        Ip4Address broadcast(ip_prefix.to_v4().to_ulong() |
                             ~(0xFFFFFFFF << (32 - plen)));
        return broadcast;
...
Ip4Address VnStaticRoute::GetSubnetAddress() const {
    if (ip_prefix.is_v4()) {
        return Address::GetIp4SubnetAddress(ip_prefix.to_v4(), plen);
    }
    return Ip4Address(0);
...
Ip6Address VnStaticRoute::GetV6SubnetAddress() const {
    if (ip_prefix.is_v6()) {
        return Address::GetIp6SubnetAddress(ip_prefix.to_v6(), plen);
    }
    return Ip6Address();
...
bool VnStaticRoute::IsSubnetMember(const IpAddress &ip) const {
    if (ip_prefix.is_v4() && ip.is_v4()) {
        return ((ip_prefix.to_v4().to_ulong() |
                 ~(0xFFFFFFFF << (32 - plen))) ==
                (ip.to_v4().to_ulong() | ~(0xFFFFFFFF << (32 - plen))));
...
VnEntry::VnEntry(Agent *agent, uuid id) :
    ...
    layer3_forwarding_(true), admin_state_(true), external_(false), table_label_(0),
    enable_rpf_(true), flood_unknown_unicast_(false), mtu_(1500),
...
bool VnEntry::ChangeHandler(Agent *agent, const DBRequest *req) {
    ...
    QosBwKey qos_bw_key(data->qos_bw_uuid_);
    QosBwEntry *qos_bw = static_cast<QosBwEntry *>
        (agent->qos_bw_table()->FindActiveEntry(&qos_bw_key));
    if (qos_bw_.get() != qos_bw) {
        qos_bw_ = qos_bw;
        resync_qos_bw |= true;
        ret = true;
    }
    if (qos_inbound_ != data->qos_inbound_) {
        qos_inbound_ = data->qos_inbound_;
        ret = true;
    }  
    if (qos_outbound_ != data->qos_outbound_) {
        qos_outbound_ = data->qos_outbound_;
        ret = true;
    } 
    if (qos_in_burst_ != data->qos_in_burst_) {
        qos_in_burst_ = data->qos_in_burst_;
        ret = true;
    }
    if (qos_out_burst_ != data->qos_out_burst_) {
        qos_out_burst_ = data->qos_out_burst_;
        ret = true;
    }
    if (qos_dscp_ != data->qos_dscp_) {
        qos_dscp_ = data->qos_dscp_;
        ret = true;
    }

    uint32_t cfg_mtu = (uint32_t)data->mtu_;
    if (mtu_ != cfg_mtu) {
        mtu_ = cfg_mtu;
        ret = true;
    }
    ...
    ret |= ApplySubnetRoute(agent, data->external_);
    ...
    static_route_ = data->static_route_;
    ret = true;
...
bool VnEntry::ApplySubnetRoute(Agent *agent, bool external) {
    bool ret = false;
    bool change = false;

    if (external_ != external) {
        external_ = external;
        change = true;
    }

    if (vrf_.get() == NULL || vrf_->GetName() == agent->linklocal_vrf_name()
         || layer3_forwarding_ == false) {
        return ret;
    }

    if (external_ == false && change == true) {
        for (unsigned int i = 0; i < ipam_.size(); ++i) {
            AddSubnetRoute(&ipam_[i]);
            ret = true;
        }
    }

    if (external_ == true && change == true) {
        for (unsigned int i = 0; i < ipam_.size(); ++i) {
            DelSubnetRoute(&ipam_[i]);
            ret = true;
        }
    }
...
IpAddress VnEntry::GetDefaultRouteFromVnHostRoutes() const {
    uint32_t i, index;
    std::string ipam;
...
VnData *VnTable::BuildData(IFMapNode *node) {
    ...
    uuid qos_policy_uuid = nil_uuid();
    string vrf_name = "";
    std::vector<VnIpam> vn_ipam;
    VnData::VnIpamDataMap vn_ipam_data;
    std::vector<VnStaticRoute> vn_static_route;
    ...
    for (DBGraphVertex::adjacency_iterator iter = node->begin(graph);
        ...
        if ((adj_node->table() == agent()->cfg()->cfg_vrf_table()) &&
            (!IsVRFServiceChainingInstance(node->name(), adj_node->name()))) {
                ...
                RoutingInstance *ri_cfg =
                static_cast<RoutingInstance *>(adj_node->GetObject());
                assert(ri_cfg);
                std::vector<autogen::StaticRouteType>::const_iterator  sr_it = ri_cfg->static_route_entries().begin();
                while (sr_it != ri_cfg->static_route_entries().end()) {
                int plen;
                boost::system::error_code ec;
                IpAddress ip;
                Ip4Address ip4;
                ec = Ip4PrefixParse(sr_it->prefix, &ip4, &plen);
                if (ec.value() == 0) {
                    ip = ip4;
                } else {
                    Ip6Address ip6;
                    ec = Inet6PrefixParse(sr_it->prefix, &ip6, &plen);
                    if (ec.value() == 0) {
                        ip = ip6;
                    } else {
                        LOG(DEBUG, "Error decoding v4/v6 Static Route address " << sr_it->prefix);
                    }
                }
                IpAddress gw = IpAddress::from_string(sr_it->next_hop, ec);
                if (ec) {
                    gw = IpAddress::from_string("0.0.0.0", ec);
                }
                vn_static_route.push_back(VnStaticRoute(ip, plen, gw));
                sr_it++;
            }
        }
        ...
        if (adj_node->table() == agent()->cfg()->cfg_qos_bw_table()) {
            QosPolicy *qos_policy_cfg = static_cast<QosPolicy *>(adj_node->GetObject());
            assert(qos_policy_cfg);
            autogen::IdPermsType id_perms = qos_policy_cfg->id_perms();
            int rule_cnt = 0;
            CfgUuidSet(id_perms.uuid.uuid_mslong, id_perms.uuid.uuid_lslong,
                      qos_policy_uuid);
            std::vector<QosBandwidthRuleType>::const_iterator  qos_policy_rule_it = qos_policy_cfg->qos_bandwidth_rule_entries().begin();
            while (qos_policy_rule_it != qos_policy_cfg->qos_bandwidth_rule_entries().end()) {
               if (++rule_cnt > MAX_QOS_RULE_NUM) {
                   assert(0);
                   break;
               }
               if (qos_policy_rule_it->direction == OUTBOUND_NAME) {
                   if (qos_policy_rule_it->max_kbps > 0) {
                       qos_outbound = qos_policy_rule_it->max_kbps;
                   }
                   if (qos_policy_rule_it->max_burst_kbps > 0) {
                       qos_out_burst = qos_policy_rule_it->max_burst_kbps;
                   }
               } else if (qos_policy_rule_it->direction == INBOUND_NAME) {
                   if (qos_policy_rule_it->max_kbps > 0) {
                       qos_inbound = qos_policy_rule_it->max_kbps;
                   }
                   if (qos_policy_rule_it->max_burst_kbps > 0) {
                       qos_in_burst = qos_policy_rule_it->max_burst_kbps;
                   }
               } else {
                   assert(0);
               }
               qos_policy_rule_it++;
               LOG(DEBUG, "vn_qos_out "<<qos_outbound<<" vn_out_burst "<<qos_out_burst<<" vn_qos_in "<<qos_inbound<<" vn_in_burst "<<qos_in_burst);
           }

            QosDscpMarkingRuleEntryType dscp_marking_rule = qos_policy_cfg->qos_dscpmarking_rule_entry();
            if (dscp_marking_rule.dscp_mark >= 0) {
                qos_dscp = dscp_marking_rule.dscp_mark;
                LOG(DEBUG, "dscp "<<qos_dscp);
            }
        }
        ...
        if (adj_node->table() == agent()->cfg()->cfg_vn_network_ipam_table()) {
            if (IFMapNode *ipam_node = FindTarget(table, adj_node,
                ...
                ipam_data.oper_dhcp_options_.set_host_routes(subnets.ipam_subnets[0].host_routes.route);
    ...
    bool external = cfg->router_external();
    int mtu = cfg->mtu();
    ...
    return new VnData(agent(), node, node->name(), acl_uuid, vrf_name,
                      ...
                      vn_ipam_data, vn_static_route,
                      cfg->properties().vxlan_network_identifier,
                      GetCfgVnId(cfg), external, cfg->id_perms().enable, enable_rpf,
                      flood_unknown_unicast, qos_inbound, qos_outbound,
                      qos_in_burst, qos_out_burst,
                      qos_dscp, mtu, qos_policy_uuid, forwarding_mode,
...
void VnTable::AddVn(const uuid &vn_uuid, const string &name,
                    ...
                    int vxlan_id, bool external, bool admin_state, bool enable_rpf,
    ...
    VnData *data = new VnData(agent(), NULL, name, acl_id, vrf_name, nil_uuid(), 
                              ...
                              vxlan_id, vn_id, external, admin_state, enable_rpf,
                              flood_unknown_unicast, Agent::NONE, nil_uuid(), nil_uuid(),
