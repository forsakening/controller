...
void OperDB::CreateDBTables(DB *db) {
    ...
    DB::RegisterFactory("db.qos_bw.0", &QosBwTable::CreateTable);
    ...
    QosBwTable *qos_bw_table;
    qos_bw_table = static_cast<QosBwTable *>(db->CreateTable("db.qos_bw.0"));;
    assert(qos_bw_table);
    agent_->set_qos_bw_table(qos_bw_table);
    qos_bw_table->set_agent(agent_);
