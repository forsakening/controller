...
public:
    ...
    struct GlobalStaticRoute {
        IpAddress prefix_;
        uint32_t plen_;
        IpAddress gw_;
        GlobalStaticRoute(const IpAddress &prefix, uint32_t plen, const IpAddress &nh):
            prefix_(prefix), plen_(plen), gw_(nh) {}
        bool operator==(const GlobalStaticRoute &rhs) const {
    ...
    int32_t get_qos_port_cnt() const { return qos_port_cnt; }
    void set_qos_port(int32_t qos_ports) {
        qos_port_cnt = qos_ports;
    }
...
private:
    ...
    typedef std::vector<autogen::RouteType> GlobalStaticRouteList;
    ...
    void UpdateGlobalStaticRouteConfig(const GlobalStaticRouteList &route_list);
    void GlobalStaticRouteChangeNotify(std::vector<GlobalStaticRoute> *old_value,
                      std::vector<GlobalStaticRoute> *new_value);
    void AddGlobalStaticRoute(std::vector<GlobalStaticRoute>::iterator &it);
    void DeleteGlobalStaticRoute(std::vector<GlobalStaticRoute>::iterator &it);
    ...
    int32_t qos_port_cnt;
    FlowAgingTimeoutMap flow_aging_timeout_map_;
    EcmpLoadBalance ecmp_load_balance_;
	std::vector<GlobalStaticRoute> global_static_route_list_;
