...
InetUnicastAgentRouteTable::FindNetRoute(const IpAddress &ip, uint32_t plen) {
...
{
    VmInterfaceKey intf_key(AgentKey::ADD_DEL_CHANGE, intf_uuid, "");
    uint32_t vxlanid = vrf_entry()->vn()->GetVxLanId();
    LocalVmRoute *data = new LocalVmRoute(intf_key, label,
                                    vxlanid, force_policy,
                                            ...
                                            bool native_encap)
{
    ...
    req.data.reset(new LocalVmRoute(intf_key, label, VxLanTable::kInvalidvxlan_id,
                                    ...
                                    //addr.is_v4()?InterfaceNHFlags::INET4:InterfaceNHFlags::INET6,
...
void InetUnicastAgentRouteTable::GetCIDRNetwork(std::vector<string> &cidrs) {
    InetRouteTree::Iterator iter = tree_.begin();
    for (; iter != tree_.end(); iter++) {
        if (((*iter)->plen() != 0)
          &&((*iter)->plen() != 32)) {
            cidrs.push_back((*iter)->ToString());
        }
    }
}
