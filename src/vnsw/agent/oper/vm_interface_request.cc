...
VmInterfaceConfigData::VmInterfaceConfigData(Agent *agent, IFMapNode *node) :
    ...
    qos_in_burst_(-1), qos_out_burst_(-1), qos_dscp_(-1), mtu_(VmInterface::kDefMtu),
    ecmp_load_balance_(),
...
#include <unistd.h>
#include <sstream>
void VmInterface::UpdateVmPortQos(int qos_inbound, int qos_outbound, int qos_in_burst, int qos_out_burst,
                         int fip_inbound,int fip_outbound, int fip_in_burst, int fip_out_busrt){
    int read_size;
    char read_buf[1024];
    ostringstream cmd;
    FILE *f = NULL;
    string cmd_path = "/usr/bin/mod_qos";
    Ip4Address ip_addr = primary_ip_addr_;

    LOG(DEBUG, "qos setting tc dev: "<<name_<< " qos_inbound "<< qos_inbound<<" qos_outboud "<<qos_outbound
        <<" qos_in_burst "<<qos_in_burst<<" qos_out_burst "<<qos_out_burst<<" fip_inbound "<<fip_inbound
        <<" fip_outbound "<<fip_outbound<<" fip_in_burst "<<fip_in_burst<<" fip_out_burst "<<fip_out_busrt
        <<" NETWORK_CIDRS="<<ip_addr.to_string());

    cmd<<cmd_path<<" "<<"DEV="<<name_<<" "<<"QOS_INBOUND="<<qos_inbound
    <<" "<<"QOS_OUTBOUND="<<qos_outbound<<" "<<"QOS_IN_BURST="<<qos_in_burst<<" "<<"QOS_OUT_BURST="<<qos_out_burst<<
    " "<<"FIP_INBOUND="<<fip_inbound<<" "<<"FIP_OUTBOUND="<<fip_outbound<<" "<<"FIP_IN_BURST="<<
    fip_in_burst<<" "<<"FIP_OUT_BURST="<<fip_out_busrt<<" "<<"NETWORK_CIDRS="<<ip_addr.to_string()<<"/32";
    f = popen(cmd.str().c_str(), "r");
    if (NULL != f) {
        read_size = fread(read_buf, 1, 1000, f);
        if (0 != read_size) {
            LOG(INFO, "tc exec info:"<<read_buf);
        }
        fclose(f);
    }
}

void VmInterface::UpdateVmPortQosVmware(int qos_inbound, int qos_outbound, const std::string &promisc_post){
    int read_size;
    char read_buf[1024];
    ostringstream cmd;
    FILE *f = NULL;
    string cmd_path = "/usr/bin/contrail-vcenter-qos";
    Ip4Address ip_addr = primary_ip_addr_;

    if (ip_addr.to_ulong() == 0){
        LOG(DEBUG, "qos update dev: "<<name_<<" NETWORK_CIDRS="<<ip_addr.to_string()<<", ignore updates!");
        return;
    }

    LOG(DEBUG, "qos setting tc dev: "<<name_<< " qos_inbound "<< qos_inbound<<" qos_outboud "<<qos_outbound<<" NETWORK_CIDRS="<<ip_addr.to_string());  

    cmd<<cmd_path<<" "<<"OP_CODE="<<"start"<<" "<<"DEV="<<promisc_post<<" "<<"ID="<<id_<<" "<<"QOS_INBOUND="<<qos_inbound
            <<" "<<"QOS_OUTBOUND="<<qos_outbound<<" "<<"NETWORK_CIDRS="<<ip_addr.to_string()<<"/32";
    f = popen(cmd.str().c_str(), "r");
    if(NULL != f){
        read_size = fread(read_buf, 1, 1000, f);
        if(0 != read_size){
...
void VmInterface::InitVmPortQosVmware(const std::string &promisc_post){
    int read_size;
    char read_buf[1024];
    ostringstream cmd;
    FILE *f = NULL;
    ...
    LOG(DEBUG, "qos setting tc init!");

    cmd<<cmd_path<<" "<<"OP_CODE="<<"init"<<" "<<"DEV="<<promisc_post;
    f = popen(cmd.str().c_str(), "r");
    if(NULL != f){
        read_size = fread(read_buf, 1, 1000, f);
        if(0 != read_size){
...
void VmInterface::DestroyVmPortQosVmware(const std::string &promisc_post){
    int read_size;
    char read_buf[1024];
    ostringstream cmd;
    FILE *f = NULL;
    ...
    LOG(DEBUG, "qos setting tc destroy!");

    cmd<<cmd_path<<" "<<"OP_CODE="<<"destroy"<<" "<<"DEV="<<promisc_post;
    f = popen(cmd.str().c_str(), "r");
    if(NULL != f){
        read_size = fread(read_buf, 1, 1000, f);
        if(0 != read_size){
...
bool VmInterface::CopyConfig(const InterfaceTable *table,
    ...
    int vmi_qos_outbound = data->qos_outbound_;
    int vmi_qos_inbound  = data->qos_inbound_;
    int vmi_qos_in_burst = data->qos_in_burst_;
    int vmi_qos_out_burst = data->qos_out_burst_;
    int32_t new_min_fip_inbound = 10485760;
    int32_t new_min_fip_outbound = 10485760;

    bool fip_qos_updated = false;
    GlobalVrouter *global_vrouter = table->agent()->oper_db()->global_vrouter();
    int32_t qos_cnt = global_vrouter->get_qos_port_cnt();
    int32_t old_qos_cnt = qos_cnt;

    int32_t vmi_qos_dscp = data ->qos_dscp_;
    uint32_t vmi_mtu = kDefMtu;
    ...
    if (table) {
        ...
        uint32_t vn_mtu = vn ? vn->GetMtu() : kDefMtu;
        vmi_mtu = data->mtu_ != kDefMtu ? data->mtu_ : vn_mtu;
        ...
        if ((NULL != vn)
            && (vn->GetVrf())){
            if (vn->GetVrf()->GetInet4UnicastRouteTable()) {
                vn->GetVrf()->GetInet4UnicastRouteTable()
                            ->GetCIDRNetwork(cidrs);
            }
        }

        int vn_qos_inbound = vn?vn->GetQosInbound() : -1;
        int vn_qos_outbound = vn?vn->GetQosOutbound() : -1;
        int vn_qos_in_burst = vn?vn->GetQosInBurst() : -1;
        int vn_qos_out_burst = vn?vn->GetQosOutBurst() : -1;

        LOG(DEBUG, "qos_inbound_: "<<qos_inbound_<< " vmi_qos_inbound "<<vmi_qos_inbound<<" qos_outbound_ "<<qos_outbound_<<
            " vmi_qos_outbound "<<vmi_qos_outbound<<" vmname "<<vm_name_<<" ipaddr "<<primary_ip_addr_.to_string());
        if ((-1 != vmi_qos_inbound && vmi_qos_inbound > vn_qos_inbound) || -1 == vmi_qos_inbound) {
            vmi_qos_inbound = (-1 == vn_qos_inbound) ? vmi_qos_inbound : vn_qos_inbound;
        }
        if ((-1 != vmi_qos_outbound && vmi_qos_outbound > vn_qos_outbound) ||-1 == vmi_qos_outbound) {
            vmi_qos_outbound = (-1 == vn_qos_outbound) ? vmi_qos_outbound : vn_qos_outbound;
        }
        if ((-1 != vmi_qos_in_burst && vmi_qos_in_burst > vn_qos_in_burst) || -1 == vmi_qos_in_burst) {
            vmi_qos_in_burst = (-1 == vn_qos_in_burst) ? vmi_qos_in_burst : vn_qos_in_burst;
        }
        if ((-1 != vmi_qos_out_burst && vmi_qos_out_burst > vn_qos_out_burst) || -1 == vmi_qos_out_burst) {
            vmi_qos_out_burst = (-1 == vn_qos_out_burst) ? vmi_qos_out_burst : vn_qos_out_burst;
        }

        int vn_qos_dscp = vn ? vn->GetQosDscp() : -1;
        LOG(DEBUG, "qos_dscp_: "<<qos_dscp_<< " vmi_qos_dscp "<<vmi_qos_dscp);
        if (vmi_qos_dscp < 0 && vn_qos_dscp >= 0){
            vmi_qos_dscp = vn_qos_dscp;
        }
    }

    if (qos_dscp_ != vmi_qos_dscp) {
        qos_dscp_ = vmi_qos_dscp >= 0 ? vmi_qos_dscp : 0;
        ret = true;
    }
    ...
    *tag_changed = AuditList<VmiReceiveRouteList,
    ...
    if (0 == new_fip_list.size() && 1 == floating_ip_list_.list_.size()) {
        new_min_fip_inbound = -1;
        new_min_fip_outbound = -1;
    } else {
        FloatingIpSet::iterator iter = floating_ip_list_.list_.begin();
        for(; iter != floating_ip_list_.list_.end(); iter++){
            if (iter->qos_inbound != -1 && iter->qos_inbound <= new_min_fip_inbound){
                new_min_fip_inbound = iter->qos_inbound;
            }
            if (iter->qos_outbound != -1 && iter->qos_outbound <= new_min_fip_outbound){
                new_min_fip_outbound = iter->qos_outbound;
            }
        }
    }

    if (new_min_fip_inbound == 10485760) new_min_fip_inbound = -1;
    if (new_min_fip_outbound == 10485760) new_min_fip_outbound = -1;

    if ((-1 == qos_inbound_ && -1 == qos_outbound_ && -1 == fip_inbound_ && -1 == fip_outbound_) && 
        (-1 != vmi_qos_inbound || -1 != vmi_qos_outbound || -1 != new_min_fip_inbound || -1 != new_min_fip_outbound)){
        qos_cnt = qos_cnt + 1;
        LOG(DEBUG, "qos_cnt +1 !!");
        global_vrouter->set_qos_port(qos_cnt);
    }else if ((-1 == vmi_qos_inbound && -1 == vmi_qos_outbound && -1 == new_min_fip_inbound && -1 == new_min_fip_outbound) && 
              (-1 != qos_inbound_ || -1 != qos_outbound_ || -1 != fip_inbound_ || -1 != fip_outbound_)){
        qos_cnt = qos_cnt - 1;
        LOG(DEBUG, "qos_cnt -1 !!");
        global_vrouter->set_qos_port(qos_cnt);
    }
    LOG(DEBUG, "old qos_cnt: "<<old_qos_cnt<<" new qos_cnt: "<< qos_cnt);
    if (qos_inbound_ != vmi_qos_inbound) {
        qos_inbound_ = vmi_qos_inbound;
        qos_updated = true;
    }
    if (qos_outbound_ != vmi_qos_outbound) {
        qos_outbound_ = vmi_qos_outbound;
        qos_updated = true;
    }
    if (qos_in_burst_!= vmi_qos_in_burst) {
        qos_in_burst_ = vmi_qos_in_burst;
        qos_updated = true;
    }
    if (qos_out_burst_ != vmi_qos_out_burst) {
        qos_out_burst_ = vmi_qos_out_burst;
        qos_updated = true;
    }
    if (qos_updated) {
        ret = true;
    ...
    if (new_min_fip_inbound != fip_inbound_||
        new_min_fip_outbound != fip_outbound_) {
        fip_inbound_= new_min_fip_inbound;
        fip_outbound_= new_min_fip_outbound;
        fip_qos_updated = true;
    }
    LOG(DEBUG, "qos_update: "<<qos_updated<< " fip_qos_updated "<<fip_qos_updated
        <<" cidr_size "<<cidrs.size()<<" vmname "<<vm_name_<<" ipaddr "<<primary_ip_addr_.to_string());
    //std::cout<<"CERTUS:new_min_fip:"<<new_min_fip_inbound<<":"<<new_min_fip_outbound<<":"<     <fip_qos_updated<<std::endl;
    if (qos_updated || fip_qos_updated) {
        ret = true;
        if (table->agent()->params()->isVmwareMode()) {
            int min_inbound = -1;
            int min_outbound = -1;
            if (qos_inbound_ < fip_inbound_) {
                if (-1 == qos_inbound_) {
                    min_inbound = fip_inbound_;
                } else {
                    min_inbound = qos_inbound_;
                }
            }else {
                if (-1 == fip_inbound_) {
                    min_inbound = qos_inbound_;
            ...
            if (qos_outbound_ < fip_outbound_) {
                if (-1 == qos_outbound_) {
                    min_outbound = fip_outbound_;
                } else {
                    min_outbound = qos_outbound_;
                }
            } else {
                if (-1 == fip_outbound_) {
                    min_outbound = qos_outbound_;
            ...
            if (0 == old_qos_cnt && 0 < qos_cnt) {
                InitVmPortQosVmware(table->agent()->params()->vmware_physical_port());
            }

            UpdateVmPortQosVmware(min_inbound, min_outbound, table->agent()->params()->vmware_physical_port());            

            if (0 < old_qos_cnt && 0 >= qos_cnt){
                DestroyVmPortQosVmware(table->agent()->params()->vmware_physical_port());
            }
        }
    }
