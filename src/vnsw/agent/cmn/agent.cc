...
void Agent::InitControllerList() {
    ...
    if (controller_list_.size() >= 1) {
        std::string controller_0 = controller_list_[0];
        int last_idx = controller_0.find_last_of(":");
        controller_0.replace(last_idx,1,"-");
        boost::split(servers, controller_0, boost::is_any_of("-"));
        ...
        if (controller_list_.size() >= 2) {
            std::string controller_1 = controller_list_[1];
            int last_idx = controller_1.find_last_of(":");
            controller_1.replace(last_idx,1,"-");
            boost::split(servers, controller_1, boost::is_any_of("-"));
...
void Agent::InitDnsList() {
    ...
    if (dns_list_.size() >= 1) {
        //guwei
        std::string dns_0 = dns_list_[0];
        int last_idx = dns_0.find_last_of(":");
        dns_0.replace(last_idx,1,"-");
        boost::split(servers, dns_0, boost::is_any_of("-"));
        ...
        if (dns_list_.size() >= 2) {
            //guwei
            std::string dns_1 = dns_list_[1];
            int last_idx = dns_1.find_last_of(":");
            dns_1.replace(last_idx,1,"-");
            boost::split(servers, dns_1, boost::is_any_of("-"));
